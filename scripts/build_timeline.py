...
def main():
    ...
    for s in specs:
        ...
        if not a or a.get('status')!='approved': raise SystemExit(f"no approved asset for {s.get('shot_id')}")
        shots.append({'shot_id':s['shot_id'],'asset_id':a['asset_id'],'start':s.get('start',0),'end':s.get('end',s.get('duration_seconds',5)),'source_start':a.get('usable_interval',[0,0])[0],'motion':s.get('motion','static'),'transition':s.get('transition','cut'),'status':'approved'})
